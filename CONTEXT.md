...
## ข้อความตอบกลับอัตโนมัติใน LINE (rewrite ตาม spec — เสร็จแล้ว 2026-06-13)
...
- **ลบ keyword auto-reply เก่า** (สั่งซื้อ/shop, สถานะ/status, help/menu, welcome catch-all) → เหลือ **fallback เดียว** `fallbackReply()` เด้งเมื่อพิมพ์ข้อความ/ส่ง sticker ที่ไม่ตรง keyword: โชว์ UNIT-01 + ลิงก์วิธีสั่งซื้อ (ต้องเว้นวรรคหลัง `[ ` ให้ LINE จับเฉพาะ URL) + ชี้พิมพ์ `[ Catalog ]` / `[ Contact Us ]`
