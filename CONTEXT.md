...
ลูกค้าเปิด LIFF → เลือกสินค้า (ดึงจาก Shopify) → กรอกที่อยู่
  ...
  → ถ้าไม่จ่ายใน 10 นาที → auto-cancel (EXPIRED) + คืนสต็อก
```
...
- ⚠️ **zero-pad ชั่วโมงก่อนเทียบ (`normalizeStamp`, fix 2026-06-14 commit `555aa8e`):** Google Sheets reformat timestamp ที่เก็บไว้เป็นชั่วโมงเลขเดี่ยวสำหรับชั่วโมง 00–09 (เช่น "2026-06-14 2:57" ไม่ใช่ "02:57") → ทำให้ "2:57" > "10:00" แบบ lexicographic ('2'>'1') → `computeEditDeadline` คิดว่าจ่ายหลัง 10:00 เลยเลื่อนเดดไลน์ไปวันถัดไป → ออเดอร์ที่จ่ายช่วงเช้า **ไม่เคยโดนล็อก**. `normalizeStamp` เติม 0 หน้าชั่วโมงก่อนเทียบทุกครั้ง (รากเดียวกับ `parseSheetTimestamp` ใน sheets.ts + `parseBkk` ใน chat-session.ts)
...
## Bot handoff overhaul + ปิดบอทแทรกแชทแมนนวล (merge + ขึ้น production แล้ว 2026-06-19)
ปรับ human-handoff ให้เร็ว+ใช้จริงง่ายขึ้น แล้วตามด้วยแก้บั๊กบอทแทรกแชทแมนนวล + ย่อการ์ดแจ้งเตือน. **แตะแค่ `app/api/webhook/route.ts` (ส่วนรับข้อความ) + `lib/chat-session.ts`** ไม่แตะ flow จ่ายเงิน/ออเดอร์/QR/SlipOK/auto-cancel/daily-pull/edit-lock
- **แจ้งเตือนเร็วขึ้น (commit `5a69290`, `c8535b5`, `5fa03ad` → merge `ef59115`):** ใช้ `after()` จาก `next/server` ย้ายงาน Sheets/expiry-sweep ออกจาก hot path (ตอบลูกค้าก่อน แล้วค่อยทำงานหนักหลัง flush response)
- **State แท็บ `chat_sessions` เดิม** 3 สถานะ: `active` (ลูกค้ากด Chat with team — legacy), `paused` (เจ้าของกดปิดบอทเอง), `seen` (มาร์คว่าเตือนเจ้าของแล้วครั้งแรก กันเตือนซ้ำ). timeout 60 นาทีทุกสถานะ. ฟังก์ชันใหม่ใน `lib/chat-session.ts`: `getSessionStatus` (อ่านครั้งเดียว), `markSeen` (สร้างแถวใหม่ครั้งแรกเท่านั้น → return true เตือนครั้งเดียว), `pauseBot`, `notifyAdminNewCustomer`
- **การ์ดแจ้งเจ้าของ 1 ใบ/ลูกค้า มีทั้งปุ่มปิด+เปิดบอท:** Flex แก้ไม่ได้หลังส่ง เลยทำการ์ดถาวรต่อลูกค้า — กด **⏸️ ปิดบอท & รับช่วงเอง** (`pause_bot`) ตอนเข้าไปขายเอง แล้วเลื่อนกลับมาการ์ดใบเดิมกด **▶️ ให้บอทกลับมา** (`resume_bot`) เมื่อเสร็จ. 2 variant: `takeover` (ส้ม — ลูกค้าทักบอทครั้งแรก, 2 ปุ่ม) / `chat` (แดง — กด Chat with team, ปุ่มจบแชทเดียว)
- **ปิดบอทแล้วเงียบรูปทั่วไป แต่สลิปจริงยังตรวจปกติ:** `handleSlipImage(..., silentOnInvalid)` — ตอน paused/active รูปที่อ่านไม่ออก = เงียบ (`[]`) แต่สลิปที่ valid/ซ้ำ/ไม่ตรงออเดอร์ ยังตอบปกติ (กันดรอปเงิน)
- **Fail-open ทุกจุด:** session gate ครอบ try/catch — ถ้า Google Sheets ล่ม จะไม่บล็อกการตอบลูกค้าหรือการอ่านสลิป (`markSeen`/`touch` พลาดก็ข้าม ไม่ throw)
...
## เงินหายซ้ำรอบ 2 (standby) — ปุ่มกู้ออเดอร์ + self-heal + Slip Log (ขึ้น production แล้ว 2026-07-02, commit `b34d19d` + `21d879c`)
**เคสจริง 2026-07-02:** ทั้งที่แก้ standby (2026-06-25 ด้านบน) ไปแล้ว เงินก็ยังหายอีก — ออเดอร์ `#UT-1OEXK9` (฿2,190): ลูกค้าโอน 13:10 บอทยกเลิก "หมดเวลา" 13:20. **หลักฐาน:** EXPIRED, ไม่มี TransRef/PaidAt, **ไม่มี** row Orphan Payments, ไม่มี chat_sessions → สลิปไม่เคยถูกประมวลผล = เจ้าของกดปุ่มเทา "แชทแบบแมนนวล" ของ LINE (standby) อีก และ standby event/รูปสลิป **LINE ส่งมาแบบ best-effort → อาจหลุดหรือดึง content ไม่ได้** (อยู่นอกการควบคุมเรา)
- **จุดที่พลาด (seam):** fix เดิม 2 อันมีรอยต่อ — standby handler ส่งสลิปไปตรวจ ✅ แต่ slip-resilience alert เจ้าของ **หลัง** `if (silentOnInvalid) return []` → ตอน standby (silent) เจ้าของ **ไม่เคยถูกเตือน** = เงินหายเงียบฝั่งเจ้าของ
- **แนวทาง (ยอมรับความจริง):** ตราบใดที่ใช้แชทแมนนวลของ LINE การตรวจ auto 100% คุมไม่ได้ (เป็นดีไซน์ LINE ไม่ใช่บั๊กเรา). เป้าหมายจริง = **"เงินไม่หายโดยไม่มีใครรู้"** → แก้ 3 ชั้น:
  1. **เตือนเจ้าของทุกครั้งที่ auto-cancel + ปุ่มกู้ 1 แตะ** (`app/api/check-expired/route.ts` → `alertOwnerOrderExpired` ใน `lib/order-sync.ts`): ทุกครั้งที่ออเดอร์ EXPIRED → push Flex การ์ดหาเจ้าของ พร้อมปุ่ม postback `action=recover_order&orderId=...`. **ไม่พึ่ง LINE ส่งสลิปซ้ำ** — เจ้าของเห็นสลิปในแชท กดปุ่มเดียวจบ
  2. **ปิด seam ฝั่ง silent** (`handleSlipImage` ใน `app/api/webhook/route.ts`): ย้าย `alertOwnerSlipFailure` มา **ก่อน** `if (silentOnInvalid) return []` ทั้ง STEP 1 (verify) และ STEP 2 (process) → standby/handoff เจ้าของได้แจ้งเตือนเสมอ. "silent" แค่ไม่ตอบลูกค้า ไม่ใช่ปิดปากบอทกับเจ้าของ
  3. **ปุ่มกู้ + self-heal** (`lib/sheets.ts`): `recoverOrderToPaid(orderId)` บังคับ EXPIRED/PENDING → PAID ใน `withLock` (idempotent กันกดซ้ำ, กัน oversell) แล้ว handler `handleRecoverOrder` (webhook) สร้าง Shopify + logStock + แจ้งลูกค้า **เหมือนสลิปจ่ายปกติเป๊ะ** (ผ่าน `after()`). **Self-heal:** ใน `claimPaymentForUser` ถ้าไม่เจอ PENDING แต่สลิป SlipOK ผ่านจริง → กู้ออเดอร์ที่เพิ่ง EXPIRED (user+ยอดตรง, ภายใน `CLAIM_RECOVER_GRACE_MIN = 60` นาที) กลับเป็น PAID อัตโนมัติ
- **helper รวม path:** `mapRowToOrder` + `markRowPaidAndCommit` (`lib/sheets.ts`) ใช้ร่วม 3 ทาง (สลิปปกติ / self-heal / ปุ่มกู้) → พฤติกรรมเดียวกัน (มาร์ค PAID + `commitSaleToStockTab` ใน-lock กัน oversell). ปุ่มกู้เชื่อสายตาเจ้าของ — **ต้องเห็นสลิปจริงก่อนกด**
- **Slip Log — ไล่จับปัญหาไม่ต้องเดา** (`logSlipEvent` ใน `lib/sheets.ts` → tab **"Slip Log"**): ทุกสลิปเขียน 1 แถว บอก stage + **mode (active/standby)**: `download_error` (LINE ไม่ส่งรูป = อาการ standby) / `slipok_error` / `invalid` / `duplicate` / `orphan` / `self_heal` / `paid` / `process_error`. รอบหน้าถ้าเงินหายอีก เปิด tab นี้เห็นทันทีว่าตกม้าตรงไหน. best-effort ไม่ throw ไม่บล็อก payment path. เพิ่ม flag `selfHealed` ใน `ClaimPaymentResult` ให้ log แยก self-heal จาก paid ปกติ
- **ยังคุมไม่ได้ (บอกเจ้าของแล้ว):** auto-detect 100% ตอนแชทแมนนวลของ LINE. ทางตัดต้นตอที่ยังไม่ยืนยัน = ตอบลูกค้าผ่าน **LINE OA Manager** แทนปุ่มเทาในแอป LINE (ต้องลองเทสว่าเลี่ยง standby ได้จริงไหม)
- เทสต์: `tsc` + `next build` ผ่าน · **loadtest C (slip-matching)** 30 concurrent → matched 30/30 distinct, wrongUser 0, double-claim 2→1 (lock ยังกันจ่ายซ้ำ) · smoke-test `logSlipEvent` เขียน tab จริงบน test sheet ผ่าน
...
- **order route → `createOrderGuarded()`** (1 ใบ = อ่าน Orders 1 + Stock 1, เช็ค available = Shopify Available − **PENDING เท่านั้น** (เลิกนับ PAID เพราะ Shopify auto-create order ตอนจ่ายแล้ว = หักให้ในตัว, นับ PAID ซ้ำ = double-count), แล้วค่อย append + log RESERVED ใน lock เดียว) — สต็อกไม่พอ → ตอบ `409 out_of_stock`, ชีต error/quota → `503 order_busy` (ให้ลูกค้า retry, ไม่สร้างออเดอร์ครึ่งๆ)
- **webhook (slip) → `claimPaymentForUser()`** — อ่าน Orders 1 + กัน transRef ซ้ำ + จับ PENDING ใหม่สุด (user+ยอดตรงเป๊ะ) + mark PAID ใน lock เดียว ปิด race "สลิปซ้ำ" และ "จ่าย 2 ใบยอดเท่ากันทับ row เดียว" พร้อมกัน. **ปิด oversell-window (2026-06-14 commit `43687cd`):** หลัง mark PAID เรียก `commitSaleToStockTab(doc, Variant IDs)` ใน lock เดียวกัน หักแท็บ Stock ("Shopify Stock"/"Available"/"Sold") ทันที — กันช่วงสั้นๆ ที่ Shopify ยังไม่ sync กลับ แล้วลูกค้าคนถัดไปเห็นของยังมี. `refreshStockTab` ใช้ **SET ค่าจาก Shopify สด (ไม่ subtract)** จึงไม่หักซ้ำ
...
- **ย้าย LINE push (Flex QR + เตือน 10 นาที) ไป background ด้วย `after()` (`next/server`)** — ตอบ HTTP กลับทันทีหลัง order ลงชีต ไม่บล็อกรอ LINE 2 round-trip. **never-silent ยังอยู่:** `after()` รันแม้ response จบแล้ว, ถ้า push พังจะเรียก `alertOwnerNotifyFailed()` แจ้งเจ้าของว่าลูกค้าอาจไม่ได้ QR (เดิม console.error เฉยๆ)
